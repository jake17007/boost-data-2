...
export default function PixelCaptionsNode() {
  ...
  const addPixelCaptions = useCallback(async (blob) => {
    setStatus('processing');
    setVideoUrl(null);

    const formData = new FormData();
    formData.append('file', blob, 'video.mp4');
    formData.append('font', 'pixel');
    ...
    try {
      ...
      console.error('Pixel captions failed:', err);
      setStatus('error');
    }
  }, []);
  ...
  useEffect(() => {
    const check = () => {
      ...
      if (data?.videoBlob && data.videoBlob !== processedBlobRef.current) {
        ...
        addPixelCaptions(data.videoBlob);
      }
    };
    check();
    return subscribe(() => check());
  }, [getSourceNodeId, addPixelCaptions]);

  return (
    <div className="pixel-captions-node">
      <Handle type="target" position={Position.Left} />

      <div className="pixel-captions-node-header">
        <div className="pixel-captions-node-icon">PX</div>
        Pixel Captions
      </div>

      <div className="pixel-captions-node-body">
        <div className="transcript-status">
          ...
          {status === 'processing' && (
            <span className="status-active">
              <span className="pulse-dot" /> Adding pixel captions...
            </span>
          )}
          {status === 'done' && <span className="status-done">Pixel captions added</span>}
        ...
        {videoUrl && (
          <div className="captions-result">
            ...
            <a className="btn btn-download" href={videoUrl} download="pixel-captioned.mp4">
              ⬇ Download
            </a>
          </div>
        )}
